...
utils.getInstallerCommand = (options) => {
  ...
  return installerCmd === 'yarn' ? { installer: 'yarn', verbs: { install: task === 'restore' ? 'install' : 'add' }, flags: { save: '--dev', exact: '--exact' } } :
    { installer: 'npm', verbs: { install: npmInstallCmd }, flags: { save: '--save-dev', exact: '--save-exact' } };
};
