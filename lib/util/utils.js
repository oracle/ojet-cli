...
/**
 * ## getExchangeUrl
 * Retrieves the exchange url
 *
 * @public
 */
utils.getExchangeUrl = () => {
  ...
  const configObj = utils.loadToolingUtil().getOraclejetConfigJson();
  const projectExchangeUrl = configObj[constants.EXCHANGE_URL_PARAM];
  if (projectExchangeUrl) {
    utils.log('Using project Exchange configuration.');
  ...
  const globalExchangeUrlFileContent = utils.readJsonAndReturnObject(
  ...
  utils.hasProperty(globalExchangeUrlFileContent, constants.EXCHANGE_GLOBAL_URL_KEY)) {
    utils.log('Using global Exchange configuration.');
 ...
 *
 * @private
 * @returns {boolean} true if is a VDOM application,
 * false otherwise
 */
utils.isVDOMApplication = () => utils.loadToolingUtil().isVDOMApplication();
  ...
  return installerCmd === 'yarn' ?
    {
      installer: 'yarn',
      verbs: {
        install: task === 'restore' ? 'install' : 'add'
      },
      flags: {
        save: '--dev',
        exact: '--exact'
...
/**
 * Writes a log message to the migration log file.
 *
 * @param {string} level - The log level (e.g. INFO, WARN, ERROR).
 * @param {string} message - The log message to write.
 * @param {string} [filePath] - Optional file path to write the log message to.
 *                              Defaults to the OJET_MIGRATE_LOG_FILE in the app directory.
 * @description This function writes a formatted log message to the specified file path,
 *              including a timestamp and log level.
 *              The log message is indented according to the log level text length.
 */
utils.writeToMigrationLogFile = (level, message, filePath) => {
  const pathToApp = utils.loadToolingUtil().destPath();
  const logFilePath = filePath || path.join(pathToApp, constants.OJET_MIGRATE_LOG_FILE);
  const timestamp = new Date().toISOString();
  const levelText = `[${timestamp}] ${level.toUpperCase().padEnd(8)}`;

  // Split the log message into individual lines
  const lines = String(message).split('\n');

  // Format each line of the log message
  const output = lines.map((line, index) => {
    // First line includes the log level text
    if (index === 0) {
      return `${levelText} ${line}`;
    }
    // Subsequent lines are indented
    return `${' '.repeat(levelText.length)} ${line}`;
  }).join('\n');

  // Append the formatted log message to the log file
  fs.appendFileSync(logFilePath, `${output}\n`);
};

/**
 * Checks if a project is a Webpack-based project by verifying the
 * existence of the Oracle JET configuration file.
 *
 * @returns {boolean} True if the project is a Webpack-based project, false otherwise.
 */
utils.isWebpackProject = () => fs.existsSync(
  path.join(utils.loadToolingUtil().destPath(), constants.OJET_CONFIG_FILE)
);
...
utils.hasLocalComponents = () => {
  ...
  [
    pathToJsComponentsFolder,
    pathToTsComponentsFolder,
    pathToVdomComponentsFolder
  ].forEach((folderPath) => {
    localComponents = [...localComponents, ...utils.getLocalComponents(folderPath)];
  });

  return localComponents.length > 0;
};

utils.getLocalComponents = (folderPath) => {
  let localComponents = [];

  if (fs.existsSync(folderPath)) {
    localComponents = fs.readdirSync(folderPath)
      .filter((item) => {
        ...
        if (toolingUtil.isLocalComponent({ component: item })) {
          return true;
        }

        const pathToComponentJson = path.join(itemPath, constants.COMPONENT_JSON);

        if (fs.existsSync(pathToComponentJson)) {
          const componentJson = JSON.parse(fs.readFileSync(pathToComponentJson, 'utf8'));

          return toolingUtil.isJETPack({ pack: item, componentJson });
        }
...
utils.getPathToViewModelsInSrc = () => {
  const pathToApp = utils.loadToolingUtil().destPath();
  const configPaths = utils.loadToolingUtil().getConfiguredPaths();
  const isTypescriptApplication = utils.isTypescriptApplication();
  const pathToViewModels = path.join(
    pathToApp,
  ...
  return pathToViewModels;
};

utils.getPathToKarmaConfigFile = () => {
  const pathToApp = utils.loadToolingUtil().destPath();

  return path.join(pathToApp, 'test-config', 'karma.conf.js');
};
