...
module.exports = function (commands) {
  ...
  if (task) {
    ...
    Object.keys(config.globalOptions).forEach((globalOptionKey) => {
      ...
      const dotsLength = titlePartLength - indent - globalOptionKey.length - 3;
      utils.log(`${space.repeat(indent)}--${globalOptionKey} ${dot.repeat(dotsLength)} ${globalOptionObject.description}\n`);
    });
