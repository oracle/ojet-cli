 ...
 */
const titlePartLength = config.help.titlePartLength;
const indent = config.help.indent;
const dot = config.help.dot;
const space = config.help.space;
...
module.exports = function (commands) {
  ...
  if (task) {
    ...
    utils.log('Task details:\n');
    _printTask(task, scope);
  } else {
    // Print all tasks, default 'ojet help'
    utils.log('Synopsis:\n');
    utils.log(`${space.repeat(indent)}ojet <task> [scope] [parameter(s)] [options]\n`);
    ...
    utils.log(`${space.repeat(indent)}ojet help <task> [<scope>]\n`);
  }
};
...
function _printTask(task, scope, taskOnly) {
  ...
  if (utils.hasProperty(taskObj, 'tasks')) {
    utils.log(`${config.help.descIndent}Tasks: ${taskObj.tasks}`);
  }
  utils.log('');
  ...
  if (!taskOnly && utils.hasProperty(taskObj, 'scopes')) {
    if (scope && utils.hasProperty(taskObj.scopes[scope], 'examples')) {
      ...
      Object.keys(taskObj.scopes).forEach((scopeItem) => {
        ...
        if (utils.hasProperty(scopeObj, 'examples')) {
...
function _printScope(task, scope) {
  ...
  if (utils.hasProperty(taskObj, 'scopes')) {
    ...
    if (utils.hasProperty(scopeObj, 'parameters')) {
      utils.log(`${config.help.descIndent}Parameter: ${scopeObj.parameters}`);
    }

    if (utils.hasProperty(scopeObj, 'default')) {
      utils.log(`${config.help.descIndent}Default: ${scopeObj.default}`);
    }
...
function _printOpts(obj) {
  Object.keys(obj).forEach((option) => {
    ...
    const dotsLength = titlePartLength - (3 * indent) - 2 - optionLength - 1;

    utils.log(`${space.repeat(3 * indent)}--${option} ${dot.repeat(dotsLength)} ${desc}`);
    if (utils.hasProperty(optionObj, 'parameters')) {
      utils.log(`${config.help.descIndent}Value: ${optionObj.parameters}`);
    }
    if (utils.hasProperty(optionObj, 'default')) {
      utils.log(`${config.help.descIndent}Default: ${optionObj.default}`);
    }
  });
}
