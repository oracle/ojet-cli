...
function _getRequestedVersion(componentName) {
  ...
  if (split.length > 2) {
    throw utils.toError('Wrong version specification: "@" can be used only once.');
  }
...
function _getPlainComponentName(componentName) {
  ...
  if (versionSymbolIndex === 0) {
    throw utils.toError('Missing component name');
  }
...
function _fetchArchive(componentMetadata) {
  ...
  return new Promise((resolve) => {
    ...
    protocol.get(connection, (response) => {
      ...
      response.on('end', () => {
        metadata.archive = Buffer.concat(buffers);
        resolve(metadata);
      });
    });
  });
...
function _unpackArchive(componentMetadata) {
  ...
  return new Promise((resolve) => {
    ...
    try {
      const zip = new AdmZip(componentMetadata.archive);
      zip.extractAllTo(componentDirPath, false);
...
function _checkForLocalComponentVersion(componentName) {
  try {
    ...
    throw utils.toError(`Can not read '${componentName}' component.json config file.`);
  }
}
...
function _packArchive(componentName) {
  const componentDirParh = componentsDirPath + componentName;
  return new Promise((resolve) => {
    utils.log(`Archiving '${componentName}' component ...`);
    const archive = '';
    try {
      const zip = new AdmZip(archive);
      zip.addLocalFolder(componentDirParh);
      zip.writeZip(`./${componentName}.zip`);
