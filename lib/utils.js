...
utils.getBuildCustomizedConfig = (options) => {
  let customConfig = {};
  const optParam = options || {};
 ...
 * @returns {boolean}
 */
utils.isVDOMApplication = () => {
  const pathToOraclejetConfig = path.join('.', CONSTANTS.APP_CONFIG_JSON);
  const oraclejetConfig = utils.readJsonAndReturnObject(pathToOraclejetConfig);
  if (utils.hasProperty(oraclejetConfig, CONSTANTS.APPLICATION_ARCHITECTURE)
    && oraclejetConfig[CONSTANTS.APPLICATION_ARCHITECTURE] === CONSTANTS.VDOM_ARCHITECTURE) {
    return true;
  }
  return false;
};
