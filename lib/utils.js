...
utils.ensureHybrid = function () {
  if (!utils.isCwdJetApp()) {
    ...
    throw utils.toError('Hybrid commands can not be used for JET web app.');
  }
};
 ...
 */
utils.fsCopySync = function (src, dest) {
  ...
  if (exists && isDirectory) {
    ...
    fs.readdirSync(src).forEach((childItemName) => {
      utils.fsCopySync(path.join(src, childItemName),
 ...
 */
utils.log.info = function (message) {
  utils.log(`\x1b[32mJET Info: ${message}\x1b[0m`);
};
...
utils.log.warning = function (message) {
  utils.log(`\x1b[33mJET Warning: ${message}\x1b[0m`);
};
